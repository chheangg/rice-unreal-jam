...
class Slot:
    ...
    def update(self, world_xyz, px_size):
...
def main():
    ...
    while True:
        ...
        for marker_id, corners_px in markers.items():
            ...
            slots[name].update(world_xyz, px_size)
        ...
        for slot in slots.values():
            ...
            osc.send_message("/obj", [slot.name, x, y, 0, slot.px_size, slot.px_size, z])
            status = "LIVE" if slot.seen else "held"
            print(f"{slot.name} x={x}mm y={y}mm z={z}mm [{status}]")
